...
import Announcement from "./Announcemment/Annountcement";
...
const Home: React.FC = () => {
  ...
  const { logged, updateCurrentUser, currentUser } = useContext(
    Context
  ) as UserContextState;

  const getTheUser = async () => {
    try {
      let res = await axios.get<User>("htpps://localhost:3000/users/user", {
